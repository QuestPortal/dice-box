    ...
    case "connect":
      ...
      worldWorkerPort.onmessage = (e) => {
        switch (e.data.action) {
          ...
          default:
            console.error(
              "action not found in physics worker from worldOffscreen worker:",
              e.data.action
            )
        }
      }
...
const init = async (data) => {
  ...
  config.throwForce =
    (config.throwForce / 2 / config.mass) * (1 + config.scale / 6)
...
const updateConfig = (options) => {
  ...
  Object.values(colliders).map((collider) => {
    collider.convexHull.setLocalScaling(
      setVector3(config.scale, config.scale, config.scale)
    )
  })
}
...
const loadModels = async ({ colliders: modelData, meshName }) => {
  ...
  if (!has_d100 && has_d10) {
    colliders[`${meshName}_d100_collider`] =
      colliders[`${meshName}_d10_collider`]
  }
}
...
const createConvexHull = (mesh) => {
  ...
  convexMesh.setLocalScaling(
    setVector3(
      mesh.scaling[0] * config.scale,
      mesh.scaling[1] * config.scale,
      mesh.scaling[2] * config.scale
    )
  )
...
const addBoxToWorld = (size, height) => {
  ...
  groundTransform.setOrigin(setVector3(0, -0.5, 0))
  ...
  ceilingTransform.setOrigin(setVector3(0, height - 0.5, 0))
  ...
  const ceilingInfo = new Ammo.btRigidBodyConstructionInfo(
    0,
    ceilingMotionState,
    ceilingShape,
    localInertia
  ...
  wallTopTransform.setOrigin(setVector3(0, 0, size / -2 - 0.5))
  ...
  const topInfo = new Ammo.btRigidBodyConstructionInfo(
    0,
    topMotionState,
    wallTopShape,
    localInertia
  ...
  wallBottomTransform.setOrigin(setVector3(0, 0, size / 2 + 0.5))
  const wallBottomShape = new Ammo.btBoxShape(
    setVector3(size * aspect, height, 1)
  )
  const bottomMotionState = new Ammo.btDefaultMotionState(wallBottomTransform)
  const bottomInfo = new Ammo.btRigidBodyConstructionInfo(
    0,
    bottomMotionState,
    wallBottomShape,
    localInertia
  ...
  wallRightTransform.setOrigin(setVector3((size * aspect) / -2 - 0.5, 0, 0))
  ...
  const rightInfo = new Ammo.btRigidBodyConstructionInfo(
    0,
    rightMotionState,
    wallRightShape,
    localInertia
  ...
  wallLeftTransform.setOrigin(setVector3((size * aspect) / 2 + 0.5, 0, 0))
  ...
  const leftInfo = new Ammo.btRigidBodyConstructionInfo(
    0,
    leftMotionState,
    wallLeftShape,
    localInertia
...
const addDie = (options) => {
  ...
  const colliderMass = colliders[comboKey]?.physicsMass || 0.1
  ...
  // clone the collider
  //console.log({ colliders, comboKey })
...
const rollDie = (die) => {
  die.setLinearVelocity(
    setVector3(
      lerp(
        -config.startPosition[0] * 0.5,
        -config.startPosition[0] * config.throwForce,
        Math.random()
      ),
      // lerp(-config.startPosition[1] * .5, -config.startPosition[1] * config.throwForce, Math.random()),
      lerp(
        -config.startPosition[1],
        -config.startPosition[1] * 2,
        Math.random()
      ),
      lerp(
        -config.startPosition[2] * 0.5,
        -config.startPosition[2] * config.throwForce,
        Math.random()
      )
    )
  )
  ...
  const scale =
    Math.abs(config.scale - 1) +
    config.scale * config.scale * (die.mass / config.mass) * 0.75
...
const update = (delta) => {
  ...
  for (let i = bodies.length - 1; i >= 0; i--) {
    ...
    if ((speed < 0.01 && tilt < 0.01) || rb.timeout < 0) {
