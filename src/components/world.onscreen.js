...
class WorldOnscreen {
  ...
  connect(port) {
    ...
    this.#physicsWorkerPort.onmessage = (e) => {
      switch (e.data.action) {
        ...
        default:
          console.error(
            "action from physicsWorker not found in offscreen worker"
          )
          break
      }
    }
  ...
  updateConfig(options) {
    ...
    if (prevConfig.shadowTransparency !== this.config.shadowTransparency) {
      this.#lights.directional.shadowGenerator.darkness =
        this.config.shadowTransparency
    }
    if (prevConfig.lightIntensity !== this.config.lightIntensity) {
      this.#lights.directional.intensity = 0.65 * this.config.lightIntensity
      this.#lights.hemispheric.intensity = 0.4 * this.config.lightIntensity
    }
  }
  ...
  renderLoop() {
    ...
    if (
      this.#sleeperCount &&
      this.#sleeperCount === Object.keys(this.#dieCache).length
    ) {
  ...
  async loadTheme(options) {
    const { theme, meshFilePath, meshName } = options
    // load the textures and create the materials needed for this theme
    await this.#themeLoader.load(options)
    ...
    if (!Object.keys(this.#meshList).includes(meshName)) {
      ...
      const colliders = await Dice.loadModels(
        { meshFilePath, meshName },
        this.#scene
      )

      if (!colliders) {
        throw new Error(
          "No colliders returned from the 3D mesh file. Low poly colliders are expected to be in the same file as the high poly dice and the mesh name contains the word 'collider'"
        )
      }
